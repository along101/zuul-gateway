...
public class WirelessExecuteRoute extends GateFilter {
	...
	public Object run() throws GateException {
		...
		try {
			...
			Collection<Header> headers = buildGateRequestHeaders(request);
			...
			//throw new GateException(errorMsg,500, "TargetUrl: ${targetUrl}\nCause: ${errorMsg}");
			throw new GateException(errorMsg,500, String.format("TargetUrl: %s\nCause: %s",targetUrl,errorMsg));
		} finally {
			tran.complete();
		}
	...
	public void setResponse(HttpResponse response) throws Exception, IOException {
		...
		for (Header header : response.getAllHeaders()) {
			if (header.getName().equalsIgnoreCase(GateHeaders.CONTENT_LENGTH)) {
				ctx.setOriginContentLength(header.getValue());
			}
			if (isValidGateResponseHeader(header.getName())) {
				 ctx.addGateResponseHeader(header.getName(), header.getValue());
			}
			if (header.getName().equalsIgnoreCase(GateHeaders.CONTENT_ENCODING)) {
	...
	public Collection<Header> buildGateRequestHeaders(HttpServletRequest request) {
		...
		for (Entry<String, String> entry : gateRequestHeaders.entrySet()) {
	...
	public HttpResponse forward(HttpClient httpclient, RequestConfig requestConfig, String verb, String url,
		...
		if (isolationStrategy == null) {
			isolationStrategy = DynamicPropertyFactory.getInstance().getStringProperty("gate.isolation.strategy.global", "SEMAPHORE").get();
		}

		long start = System.currentTimeMillis();
		try {
			if ("THREAD".equalsIgnoreCase(isolationStrategy)) {
				return new GateRequestCommandForThreadIsolation(httpclient, httpUriRequest, groupName, routeName).execute();
			} else {
				return new GateRequestCommandForSemaphoreIsolation(httpclient, httpUriRequest, groupName, routeName).execute();
			}
		} finally {
	...
	public boolean isValidGateResponseHeader(String name) {
	...
	private RequestConfig buildRequestConfig(String routName, String groupName) {
		...
		if (socketTimeout == 0) {
			socketTimeout = DynamicPropertyFactory.getInstance().getIntProperty("gate.socket.timeout.global", 1000)
					.get();
		}
