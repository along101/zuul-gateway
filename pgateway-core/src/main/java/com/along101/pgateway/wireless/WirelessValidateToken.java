...
public class WirelessValidateToken extends GateFilter {

	private static Logger logger = LoggerFactory.getLogger(WirelessValidateToken.class);
	...
	public Object run() throws GateException {
		...
		try{
			...
			if (gateway == null) {
				//throwException(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "GTW-UNKNOW", "已加载的服务信息丢失");
				fomatResponse(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "GTW-UNKNOW", "已加载的服务信息丢失");
				return null;
			}
			...
			if (gateway.getValidateToken()) {
				...
				if (!headers.hasMoreElements()) {
					//throwException(HttpServletResponse.SC_BAD_REQUEST, "GTW-BRQ-NOHEADER", "访问令牌不存在");
					fomatResponse(HttpServletResponse.SC_BAD_REQUEST, "GTW-BRQ-NOHEADER", "访问令牌不存在");
					return null;
				}

				String token = StringUtils.trim(headers.nextElement());
				if (StringUtils.isEmpty(token)) {
					//throwException(HttpServletResponse.SC_BAD_REQUEST, "GTW-BRQ-NOHEADER", "访问令牌内容为空");
					fomatResponse(HttpServletResponse.SC_BAD_REQUEST, "GTW-BRQ-NOHEADER", "访问令牌内容为空");
					return null;
				}
				...
				try {
					...
					if (result.getResult() != 0) {
						//throwException(HttpServletResponse.SC_BAD_REQUEST, "GTW-BRQ-INVALIDTOKEN",String.format("令牌校验失败：‘%s’", result.getResultMessage()));
						fomatResponse(HttpServletResponse.SC_BAD_REQUEST, "GTW-BRQ-INVALIDTOKEN",String.format("令牌校验失败：‘%s’", result.getResultMessage()));
						return null;
					}
					...
					Cat.logError(
							String.format("令牌服务响应异常！token=%s, deviceID=%s, error=%s", token, deviceId, ex.getMessage()),
							ex);
					throwException(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "GTW-SRV-TOKENSERVICEERROR",
							"令牌服务响应异常");
				}
			}
			// RequestContext.getCurrentContext().getResponse().setHeader("X-ALONG-TIMESTAMP",
	...
	private TokenResult validateToken(TokenRequest request) throws Exception {
		...
		url = url + "/Validate";
		Transaction tran = Cat.newTransaction("Call", url);
		try {
