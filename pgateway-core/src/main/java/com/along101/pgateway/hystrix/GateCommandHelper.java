...
public class GateCommandHelper {
    ...
    public static int getRibbonConnectTimeout(String groupName, String routeName) {
		...
		if (connectTimeout == 0) {
			connectTimeout = DynamicPropertyFactory.getInstance().getIntProperty("gate.connect.timeout.global", 20000)
					.get();
		}
    ...
    public static boolean getRibbonRequestSpecificRetryOn(String groupName, String routeName) {
		...
		if (!nextTry) {
			nextTry = DynamicPropertyFactory.getInstance().getBooleanProperty("gate.socket.next.try.global", true).get();
		}
		return nextTry;
    }
    ...
    public static boolean getRibbonOkToRetryOnAllOperations(String groupName, String routeName) {
		...
		if (!sameTry) {
			sameTry = DynamicPropertyFactory.getInstance().getBooleanProperty("gate.socket.same.try.global", true).get();
		}
		return sameTry;
    }
}