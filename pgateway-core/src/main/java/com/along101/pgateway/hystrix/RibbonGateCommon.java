...
public abstract class RibbonGateCommon extends HystrixCommand<HttpResponse> {
	...
	public RibbonGateCommon(AbstractLoadBalancerAwareClient client,HttpRequest request,String serviceName, String commandGroup, String commandKey,Setter setter) {
		super(setter);
	...
	protected RestClient getRestClient() throws GateException {
		Application application = DiscoveryManager.getInstance().getDiscoveryClient().getApplication(serviceName);
		if (application == null) {
			throw new GateException( "Service-NotFoud",HttpServletResponse.SC_NOT_FOUND, serviceName + "服务未找到");
		}
...
//		int ruleLoad = GateCommandHelper.getLoadBalanceRule(commandGroup, commandKey);
//		if (ruleLoad == 2) {
