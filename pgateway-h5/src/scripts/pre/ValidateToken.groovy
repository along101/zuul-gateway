...
public class ValidateToken extends GateFilter {
	...
	private void throwException(Throwable t,int httpStatus, String code, String message) throws GateException {
		...
		throw new GateException(t,code, httpStatus, message);
	}
	...
	public Object run() throws GateException {
		...
		try {
			...
			if (gateway.getValidateToken()) {
				...
				String deviceId = "";
				for(Cookie cookie:cookies){
					if(cookie.name.equalsIgnoreCase("deviceId")){
						deviceId = cookie.value;
						break;
					}
				}
				
				deviceId = StringUtils.trim(deviceId);
				...
				try {
					...
					ctx.addGateRequestHeader("X-ALONG-OPENAPIGATEWAY","1");
					...
					throwException(ex,HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "GTW-SRV-TOKENSERVICEERROR",new String("令牌服务响应异常".getBytes(),"utf-8"));
				}
			}
