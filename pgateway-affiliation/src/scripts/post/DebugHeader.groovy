...
class DebugHeader extends GateFilter {
    static final DynamicBooleanProperty INCLUDE_DEBUG_HEADER =
            DynamicPropertyFactory.getInstance().getBooleanProperty("gate.include.debug.header", false);

    static final DynamicBooleanProperty INCLUDE_ROUTE_URL_HEADER =
            DynamicPropertyFactory.getInstance().getBooleanProperty("gate.include-route-url-header", true);
    ...
    void addStandardResponseHeaders(HttpServletRequest req, HttpServletResponse res) {
        ...
        if (INCLUDE_DEBUG_HEADER.get()) {
            ...
            headers.add(new Pair("X-Gate-Debug-Header", debugHeader));
        }
    }
