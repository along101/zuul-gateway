...
public class SearchRouteForAPI extends GateFilter {
	...
	public Object run() throws GateException {
		...
		try{
			...
			StringBuilder postUrl = null;
			try {
				ctx.setServiceName(serviceName);
				
				postUrl = new StringBuilder(getRouteUrl(serviceName) + request.getRequestURI());
			...
			ctx.setRouteName("soa."+serviceName);
			ctx.setRouteGroup("soa."+serviceName);
	...
	private String getRouteUrl(String router) throws Exception {
		return DynamicPropertyFactory.getInstance().getStringProperty(router.toLowerCase(),null).get();
	}
}