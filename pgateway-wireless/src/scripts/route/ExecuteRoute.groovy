...
public class ExecuteRoute extends GateFilter{
	...
	private static final DynamicBooleanProperty DEBUG_ROUTE = DynamicPropertyFactory.getInstance().getBooleanProperty("gate.debug.thread.route", false);
		...
		while (headerNames.hasMoreElements()) {
			...
			if (isValidGateRequestHeader(name)) {
				...
				if("X-Forwarded-For".equalsIgnoreCase(name)){					
					headersMap.put("X-ALONG-IPADDRESS",new BasicHeader("X-ALONG-IPADDRESS", StringUtils.substringBefore(value, ",")));
				}else if("User-Agent".equalsIgnoreCase(name)){
					headersMap.put("X-ALONG-USERAGENT",new BasicHeader("X-ALONG-USERAGENT", value));
				}
			}
